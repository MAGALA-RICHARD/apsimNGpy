...
def extract_docstrings(file_path):
    ...
    try:
        ...
        docstrings = {}
        current_class = None  # Variable to track the current class we're processing
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.ClassDef):
                ...
                if class_docstring:
                    docstrings[f"Class: {node.name}"] = class_docstring.strip('\"\'')
                # Set the current class to track methods
                current_class = node.name

            # Process Function Definitions (methods or functions)
            elif isinstance(node, ast.FunctionDef):
                docstring = ast.get_docstring(node)
                if docstring:
                    if current_class:
                        # It's a method inside a class
                        key = f"Method: {current_class}.{node.name}"
                    else:
                        # It's a function at the module level
            ...
            # If the child is not a method or class, reset current_class when leaving a class scope
            elif isinstance(node, ast.Module) and not any(isinstance(child, ast.ClassDef) for child in node.body):
                current_class = None

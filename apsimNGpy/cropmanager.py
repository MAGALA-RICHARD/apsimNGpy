...
def InsertCroppingSystems(path2apsimx, rotation = "Maize", NAmount = 420, nFraction = 1, fertilizerdate = '30-may', tillage_param =[0.55, 250], file_name = False, action_mapping =None):
      try:
        '''
        ...
        fertilizer = [{'Key': 'Crop', 'Value': '[Maize]'}, {'Key': 'FertiliserType', 'Value': 'UreaN'}, {'Key': 'Fraction', 'Value': '1'}, {'Key': 'Amount', 'Value': NAmount},
                {'Key': 'FertiliserDates', 'Value': fertilizerdate}, {'Key': 'AmountType', 'Value': 'True'}, { "Key": "Fraction", "Value": nFraction },]
                                  ...
                                  for counter, root in enumerate(app_ap["Children"][coresimulationNode]["Children"][fieldzone]["Children"]):
                                        for key in root:
                                         if root[key] in list(action_mapping.keys()):
                                            ...
                                            app_ap["Children"][coresimulationNode]["Children"][fieldzone]["Children"][ mapping_index]['Parameters'] = action

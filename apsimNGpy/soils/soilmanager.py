...
from apsimNGpy.core_utils.soil_lay_calculator import gen_layer_bounds_from_single_thickness, layer_boundaries
import requests
import xmltodict
...
class OrganiseSoilProfile:
    ...
    def cal_missingFromSurgo(self, curveparam_a=0, curveparam_b=0.2, crops=("Wheat", "Maize", "Soybean", "Rye"),
                             ...
                             top_finert=0.65, top_fom=180, top_fbiom=0.04, fom_cnr=40, soil_cnr=12,
        ...
        Urea = pd.DataFrame(
            {'Depth': self.depths, 'Thickness': self.thickness_values,
             'InitialValues': np.full(len(self.depths), top_urea)})
        ...
        if isinstance(self.CSR, (list, np.ndarray, pd.Series)):
            ...
            CSR = None
        # result_df = pd.concat(frame, join='outer', axis=1)
        # ------------------------ final dict data ------------------------

...
from apsimNGpy.core.runner import run_from_dir, collect_db_from_dir, aggregate_data, \
    dir_simulations_to_sql, dir_simulations_to_dfs # (unused here but kept if you need later)
...
class TestRunner(unittest.TestCase):
    def setUp(self):
        ...
        self.size = 20
        self.Dir_mix = mimic_multiple_files(out_file=f'out_file_{name}', size=self.size, mix=True)
        self.Dir_no_mix = mimic_multiple_files(out_file=f'out_file2_{name}', size=self.size, mix=False)

    def test_run_from_dir_mixed_files(self):
        df = run_from_dir(dir_path=self.Dir_mix, pattern="*__.apsimx", recursive=False, write_tocsv=False,
                          tables='Report',
                          cpu_count=10)
    ...
    def test_data_types(self):
        ...
        pds = all(isinstance(df, DataFrame) for df in df.values())
        empty_or_not = all(df.empty for df in df.values())
        self.assertTrue(pds)
        self.assertFalse(empty_or_not)

    def test_run_from_dir_not_mixed_files(self):
        df = run_from_dir(dir_path=self.Dir_no_mix, pattern="*__.apsimx", recursive=False, write_tocsv=False,
                          tables='Report',
                          cpu_count=10)
        keys = df.keys().__len__()
        self.assertEqual(keys, 1)
        collection_ids = list(df.values())[
            0].CollectionID.unique()  # here it is one data frame because files are homogenous

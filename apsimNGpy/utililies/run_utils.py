...
def read_sqlite_simulation_data(sqlite_db):
    """
    ...
    with sqlite3.connect(sqlite_db) as conn:
        ...
        for i in table_names:
            ...
            # remove these
        excluded_tables = ['_InitialConditions', '_Messages', '_Checkpoints', '_Units']
        ...
        # create data from dictionary to keep all the tables
        dataframe_dict = {}

        for table_name in table_list:
            query = select_template.format(table_list=table_name)
            dataframe_dict[tname] = pd.read_sql(query, conn)

        return dataframe_dict


def read_simulation(datastore, report_name=None):
    """ returns all data frame the available report tables"""
    dataframe_dict = read_sqlite_simulation_data(datastore)
...
def run(named_tuple_data, clean=False, multithread=True, read_db=False):
    ...
    e = runmodel.Run()
    if read_db:
        data = read_simulation(named_tuple_data.datastore)
        return data

...
```
replicate_file(self: unknown, k: int, path:  , tag: str)
...
```
recompile_edited_model(self: unknown, out_path:  )
...
```
extract_soil_physical(self: unknown, simulations:  )
...
```
extract_any_soil_physical(self: unknown, parameter: unknown, simulations:  )
